...
*Tổng hợp ngắn gọn lại! : Như vậy để thiết lập cho một Profile ta sẽ sửa trong file template tương ứng của Profile nằm tại ```/etc/authselect/custom/<profile_name>``` rồi chạy ```authselect apply-changes```. Lệnh này sẽ sinh ra file cấu hình của PAM tại ```/etc/authselect```. File này được link vào thưc mục cấu hình của PAM tại ```/etc/pam.d/```. Lúc này cấu hình sẽ được thực sự áp dụng vào hệ thống!*
